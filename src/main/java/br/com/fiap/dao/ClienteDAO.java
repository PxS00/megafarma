...
public class ClienteDAO {
    public ArrayList<ClienteTO> findAll() {
        ...
        String sql = "select codigo,nome,cpf,email,data_de_nascimento,senha from ddd_clientes order by codigo";
    ...
    public ClienteTO findByCodigo(long codigo){
        ClienteTO cliente = new ClienteTO();
        String sql = "select codigo,nome,cpf,email,data_de_nascimento,senha from ddd_clientes where codigo = ?";
    ...
    public ClienteTO save(ClienteTO cliente) {
        String sql = "insert into ddd_clientes(nome, cpf, email, data_de_nascimento, senha) values(?,?,?,?,?)";
        try (PreparedStatement ps = ConnectionFactory.getConnection().prepareStatement(sql)) {
    ...
    public ClienteTO update(ClienteTO cliente) {
        String sql = "update ddd_clientes set nome=?, cpf=?, email=?, data_de_nascimento=?, senha=? where codigo=?";
        try (PreparedStatement ps = ConnectionFactory.getConnection().prepareStatement(sql)) {
            ps.setString(1, cliente.getNome());
            ps.setString(2, cliente.getCpf());
            ps.setString(3, cliente.getEmail());
